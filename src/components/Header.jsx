...
const Header = () => {
  ...
  const adminEmails = ["mueezimran1@gmail.com", "aw1482072@gmail.com"];

  return (
    <>
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#ededed] bg-[#FFB8E0] px-4 md:px-10 py-3 relative">
        <div className="flex items-center gap-4 md:gap-8">
          <Link to="/" className="flex items-center gap-2 md:gap-4 text-[#141414]">
            <h2 className="text-[#141414] text-lg font-bold leading-tight tracking-[-0.015em]">Laliza World 🎀</h2>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-5 lg:gap-9">
            ...
            <Link to="/products?category=Makeup" className="text-[#141414] text-sm font-medium transition">Makeup ˚⊹♡</Link>
            <Link to="/products?category=Nails" className="text-[#141414] text-sm font-medium  transition">
Nails ⋆｡𖦹°</Link>
            <Link to="/products?category=Lip glosses" className="text-[#141414] text-sm font-medium  transition">Lip glosses ˚⋆𐙚｡</Link>
            <Link to="/products?category=Hair products" className="text-[#141414] text-sm font-medium  transition">Hair products˚⋆𐙚｡</Link>
...
#FFB8E0] items-center justify-center pl-4 rounded-l-lg border-r-0">
...
#FFB8E0] focus:border-none h-full placeholder:text-neutral-500 px-4 rounded-l-none border-l-0 pl-2 text-base font-normal leading-normal"
                value={searchQuery}
...
#FFB8E0] text-[#141414] hover:bg-[#FFDCDC] transition">🛒 Cart</button>
            {user ? (
              <>
                <button onClick={handleLogout} className="px-4 h-10 rounded-lg bg-[#
#FFB8E0] text-[#141414] text-sm font-semibold hover:bg-[#
#FFB8E0] transition">Logout</button>
...
#FFB8E0] text-white text-sm font-semibold hover:opacity-90 transition"
              >
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg z-50 border-t border-[#ededed]">
            <div className="px-4 py-2 space-y-2">
              <div className="flex items-center gap-2 rounded-lg bg-[#FFB8E0] px-3 py-2">
              ...
              <Link to="/products?category=Makeup" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Makeup ˚⊹♡</Link>
              <Link to="/products?category=Nails" onClick={() => setIsMenuOpen(false)} className="blocsk px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Nails ⋆｡𖦹°
</Link>
              <Link to="/products?category=Lip glosses" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">
Lip glosses˚⋆𐙚｡
</Link>
              <Link to="/products?category=Hair products" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Hair products ｡°✩
</Link>
